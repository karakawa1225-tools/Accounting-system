...
{`会計設定 → マスタ(CSV可) → 期首残高 → 売掛(部署選択)・買掛(買掛金/外注)・入出金 → 月次PDF・CSV`}
      ...
      <div className="grid gap-6 md:grid-cols-1">
        ...
        <ScreenCard
          ...
          steps={[
            "「売上登録」で先に部署（施工部・機工部）を選び、売掛・売上の計上先を決めます。",
            "履歴の「編集」からも部署を変更できます（消込済み売上・入金は不可）。",
            "月次 PDF / CSV は履歴横の部署プルダウンで対象を選びます。",
          ]}
        />
        <ScreenCard
          icon="📤"
          title="買掛管理（買掛金）"
          href="/admin/payables/kaikake"
          steps={[
            "仕入登録で買掛金・仕入高に計上します（従来の買掛と同じ勘定）。",
            "支払消込・月別支払 PDF/CSV・仕入先振込リストは買掛金のみ対象です。",
          ]}
        />
        <ScreenCard
          icon="📤"
          title="買掛管理（外注費）"
          href="/admin/payables/gaichu"
          steps={[
            "外注費登録で未払外注費・外注費に計上します。",
            "支払消込や月次出力は外注費用の勘定のみ集計されます。",
          ]}
        />
        <ScreenCard
          icon="📋"
